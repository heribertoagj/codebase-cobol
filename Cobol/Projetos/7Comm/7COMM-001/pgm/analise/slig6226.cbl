      ...
      *---------------------------------------------------------------*
      *                                                               *
      *    PROGRAMADORA : ADRIANA BARON                - CPM/FPOLIS   *
      *    ANALISTA CPM : MICHELLI VENTURA             - CPM/FPOLIS   *
      *    ANALISTA     : DANIEL ARCE - DTS CONSULTING - GRUPO 73     *
      *    DATA         : 12/09/2005                                  *
      *                                                               *
      *    OBJETIVO     :                                             *
      *      ALTERAR O TRATAMENTO DOS ARQUIVOS MVTCHQSR E EMTCHEQ     *
      *      QUE UTILIZAM HOJE A INC I#SLIG28 PARA O NOVO LAY-OUT     *
      *      DE OCORRENCIAS HISTORICAS I#CLIGC0. ATUALIZAR AS         *
      *      INFORMACOES CADASTRAIS CONFORME O PROCESSO ATUAL DO      *
      *      PROGRAMA.                                                *
      *                                                               *
RAQ   *================================================================*
RAQ   *                    A L T E R A C A O                           *
RAQ   *----------------------------------------------------------------*
RAQ   *    ANALISTAS...:   RAQUEL / REGINALDO          - DTS           *
RAQ   *    DATA........:   OUTUBRO/2005                                *
RAQ   *----------------------------------------------------------------*
RAQ   *    OBJETIVO....:   IDENTIFICAR COMO JURIDICA QUANDO NUMERO DO  *
RAQ   *      CPF ESTIVER ZERADO.                                       *
RAQ   *================================================================*
AD1006*                    A L T E R A C A O                           *
AD1006*----------------------------------------------------------------*
AD1006*    ANALISTAS...:   ROSANA / JOSE ADELMO        - DTS           *
AD1006*    DATA........:   OUTUBRO/2006                                *
AD1006*----------------------------------------------------------------*
AD1006*    OBJETIVO....:   CORRIGIR PROCESSO DE IDENTIFICACAO DO CNPJ/ *
AD1006*                    CPF DO CLIENTE                              *
AD1006*================================================================*
      ...
      *----------------------------------------------------------------*
       77  FILLER                      PIC  X(050)         VALUE
           '*** SLIG6226 - INICIO DA AREA DE WORKING ***'.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       ...
       77  AC-01                       PIC  9(015) COMP-3  VALUE ZEROS.
      ...
      *----------------------------------------------------------------*
       ...
       77  WRK-CADCLIE                 PIC  X(008)         VALUE
           'CADCLIE'.
       77  WRK-MVTCHQSR                PIC  X(008)         VALUE
           'MVTCHQSR'.
       77  WRK-EMITCHEQ                PIC  X(008)         VALUE
           'EMITCHEQ'.
       77  WRK-CGC-CPF-CLI             PIC  X(009)         VALUE ZEROS.
CNPJ7C 77  WRK-CGC-CPF-CLI-N  REDEFINES
CNPJ7C     WRK-CGC-CPF-CLI             PIC  9(009).
       77  WRK-FILIAL-CLI              PIC  X(004)         VALUE ZEROS.
CNPJ7C 77  WRK-FILIAL-CLI-N   REDEFINES
CNPJ7C     WRK-FILIAL-CLI              PIC  9(004).
      ...
      *----------------------------------------------------------------*
       01  FILLER                      PIC  X(050)         VALUE
           '*** AREA PARA OS ARQUIVOS MVTCHQSR E EMITCHEQ ***'.
      *----------------------------------------------------------------*
...
-INC POL7100C
      ...
      *----------------------------------------------------------------*
       01  FILLER                      PIC  X(050)         VALUE
           '*** SLIG6226 - FIM DA AREA DE WORKING ***'.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
           ...
           PERFORM 111000-TESTAR-RC-MVTCHQSR.

           PERFORM 112000-TESTAR-RC-EMITCHEQ.
      ...
      *----------------------------------------------------------------*
           EJECT
      *----------------------------------------------------------------*
       111000-TESTAR-RC-MVTCHQSR       SECTION.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       112000-TESTAR-RC-EMITCHEQ       SECTION.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
           ...
           IF WRK-FS-MVTCHQSR   EQUAL  'S'  AND
              ACU-LIDOS-MVTCHQSR    NOT GREATER ZEROS
               DISPLAY '***************** SLIG6226 *****************'
               DISPLAY '*                                          *'
...
P/TEST*    ADD 1 TO AC-01.
P/TEST*    IF AC-01 = 10
P/TEST*        MOVE  'S'               TO WRK-FS-MVTCHQSR
P/TEST*        MOVE HIGH-VALUES        TO WRK-CHV-ATU
P/TEST*    END-IF.

           CALL 'BRAD0300'  USING  WRK-MVTCHQSR
                                   REG-CHEQUE-OCOR-HISTORICA.
           IF  RETURN-CODE             EQUAL 04
               MOVE  'S'               TO WRK-FS-MVTCHQSR
...
AD1006     MOVE SLIGC0-CGC-CPF-CLI-ST  TO WRK-CGC-CPF-CLI.
           MOVE SLIGC0-BCO-CLI         TO WRK-BCO-ATU
           MOVE SLIGC0-AGENCIA-CLI     TO WRK-AGE-ATU
           MOVE SLIGC0-CONTA-CLI       TO WRK-CTA-ATU
...
AD1006*                                 WRK-CGC-CPF-CLI
              ...
              IF WRK-CGC-CPF-CLI   NOT EQUAL ZEROS AND SPACES
      ...
      *--  FORMATA CAMPOS DO CLIENTE PARA GRAVACAO  -------------------*
           ...
           MOVE WRK-GRP-ECONOMICO-CLI    TO SLIGC0-GRP-ECONOMICO-CLI.

CNPJ7C     IF  (WRK-CGC-CPF-CLI  NUMERIC) AND
CNPJ7C         (WRK-FILIAL-CLI   NUMERIC)
CNPJ7C          MOVE WRK-CGC-CPF-CLI-N   TO SLIGC0-CGC-CPF-CLI
CNPJ7C          MOVE WRK-FILIAL-CLI-N    TO SLIGC0-FILIAL-CLI
CNPJ7C          MOVE WRK-CONTROLE-CLI    TO SLIGC0-CONTROLE-CLI
CNPJ7C     ELSE
CNPJ7C          MOVE ZEROS               TO SLIGC0-CGC-CPF-CLI
CNPJ7C                                      SLIGC0-FILIAL-CLI
CNPJ7C                                      SLIGC0-CONTROLE-CLI
CNPJ7C     END-IF

           MOVE WRK-CGC-CPF-CLI          TO SLIGC0-CGC-CPF-CLI-ST.
           MOVE WRK-FILIAL-CLI           TO SLIGC0-FILIAL-CLI-ST.
           MOVE WRK-CONTROLE-CLI         TO SLIGC0-CONTROLE-CLI-ST.
           MOVE WRK-NOME-CLI             TO SLIGC0-NOME-CLI.
           MOVE WRK-BCO-DIR-REGIONAL     TO SLIGC0-BCO-DIR-REGIONAL.
           MOVE WRK-DIR-REGIONAL         TO SLIGC0-DIR-REGIONAL.
           MOVE WRK-BCO-GERC-REGIONAL    TO SLIGC0-BCO-GERC-REGIONAL.
           MOVE WRK-GERC-REGIONAL        TO SLIGC0-GERC-REGIONAL.

      *--  FORMATA NOME EMITENTE CASO CNPJ/CPF SEJA IGUAL AO CLIENTE --*

           IF SLIGC0-CGC-CPF-CLI-ST     EQUAL SLIGC0-CGC-CPF-EMITT-ST
           AND
              SLIGC0-FILIAL-CLI-ST      EQUAL SLIGC0-FILIAL-EMITT-ST
              AND
              SLIGC0-CONTROLE-CLI-ST    EQUAL SLIGC0-CONTROLE-EMITT-ST
              AND
              SLIGC0-NOME-CLI        NOT EQUAL SPACES             AND
              SLIGC0-NOME-CLI        NOT EQUAL
                                     'CLIENTE NAO ENCONTRADO'     AND
              SLIGC0-NOME-EMITT      NOT EQUAL SLIGC0-NOME-CLI
              MOVE SLIGC0-NOME-CLI       TO SLIGC0-NOME-EMITT
           END-IF.

      *--  FORMATA TIPO DE PESSOA CLIENTE  ----------------------------*

RAQ        IF SLIGC0-CGC-CPF-CLI-ST     EQUAL '000000000' OR SPACES
RAQ           MOVE 'J'                      TO SLIGC0-TIPO-PESSOA-CLI
RAQ        ELSE
              IF (SLIGC0-CGC-CPF-CLI-ST
              NOT EQUAL ZEROS AND SPACES) AND
                (SLIGC0-FILIAL-CLI-ST
                 EQUAL ZEROS OR SPACES)
                 MOVE 'F'                   TO SLIGC0-TIPO-PESSOA-CLI
              ELSE
                 MOVE 'J'                   TO SLIGC0-TIPO-PESSOA-CLI
              END-IF
RAQ        END-IF.

      *--  ACUMULA GERENCIA REGIONAL CLIENTE ZERADA  ------------------*

           IF SLIGC0-BCO-GERC-REGIONAL EQUAL ZEROS AND
              SLIGC0-GERC-REGIONAL     EQUAL ZEROS
      ...
      *--  ACUMULA DIRETORIA REGIONAL CLIENTE ZERADA  -----------------*
           ...
           IF SLIGC0-BCO-DIR-REGIONAL  EQUAL ZEROS AND
              SLIGC0-DIR-REGIONAL      EQUAL ZEROS
      ...
      *--  ACUMULA GRUPO ECONOMICO CLIENTE ZERADO  --------------------*
           ...
           IF SLIGC0-GRP-ECONOMICO-CLI EQUAL ZEROS
      ...
      *--  ACUMULA NOME CLIENETE NAO ECONTRADO E SPACES ---------------*
           ...
           IF SLIGC0-NOME-CLI         EQUAL 'CLIENTE NAO ENCONTRADO'
              ADD 1                      TO ACU-CLI-NOME-N-ENC-OUT
           ELSE
              IF SLIGC0-NOME-CLI      EQUAL SPACES
      ...
      *--  GRAVA REGISTRO EM EMITCHEQ  --------------------------------*
           ...
           CALL 'BRAD0315'           USING WRK-EMITCHEQ
                                           REG-CHEQUE-OCOR-HISTORICA.

           IF  RETURN-CODE             NOT EQUAL ZEROS
                 MOVE 'APL'              TO ERR-TIPO-ACESSO
                 MOVE 'BRAD0315'         TO WRK-NOME-BRAD
                 MOVE 'EMICHEQ'          TO WRK-NOME-ARQ
...
.,            ADD  1                  TO ACU-CLI-NOME-CPF-ATLZ
      ...
      *----------------------------------------------------------------*
           ...
           MOVE SLIGC0-BCO-CLI         TO COMU-COD-EMPR.
           MOVE SLIGC0-AGENCIA-CLI     TO COMU-COD-DEPDC.
           ...
           IF RETURN-CODE              EQUAL ZEROS
              ...
              MOVE SLIGC0-BCO-CLI TO WRK-BCO-DIR-REGIONAL
      ...
      *----------------------------------------------------------------*
           ...
           MOVE 'SLIG6226'             TO WRK-9023-NOME-PGM.
           MOVE 'B'                    TO WRK-9023-AMBIENTE.

           MOVE SLIGC0-BCO-CLI         TO WRK-9023-BANCO.
           MOVE SLIGC0-AGENCIA-CLI     TO WRK-9023-AGENCIA.
      ...
      *----------------------------------------------------------------*
           ...
           MOVE 'SLIG6226'             TO GRUPW01S-PGM-CHAMADOR.
      ...
      *----------------------------------------------------------------*
           ...
           IF  ACU-GRAVA-EMITCHEQ      GREATER ZEROS
               CALL 'FECHAARQ'         USING WRK-EMITCHEQ
           END-IF.
