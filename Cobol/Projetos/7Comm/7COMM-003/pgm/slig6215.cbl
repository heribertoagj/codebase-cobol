...
-INC I#SLIG74
      ...
      *----------------------------------------------------------------*
       77  FILLER                      PIC  X(050)         VALUE
           '*** SLIG6215 - INICIO DA AREA DE WORKING ***'.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       ...
       77  ACU-GRUP201C-GRP-ENC        PIC  9(015) COMP-3  VALUE ZEROS.
       77  ACU-GRUP201C-GRP-N-ENC      PIC  9(015) COMP-3  VALUE ZEROS.
       77  ACU-GRUP201C-ERRO           PIC  9(015) COMP-3  VALUE ZEROS.
       ...
       77  WRK-GRUP201C-MSG-ERRO       PIC  X(035)         VALUE ZEROS.
      ...
      *----------------------------------------------------------------*
       ...
       01  LINTOT13.
           ...
           03  FILLER                  PIC  X(010)         VALUE SPACES.
           03  FILLER                  PIC  X(033)         VALUE
               'TOT ACESSOS AO GRUP201C        : '.
           03  LT13-TOT-ACESSO-GRUP201C PIC  ZZZ.ZZZ.ZZZ.ZZZ.ZZ9.

       01  LINTOT14.
           03  FILLER                  PIC  X(001)         VALUE '0'.
           03  FILLER                  PIC  X(010)         VALUE SPACES.
           03  FILLER                  PIC  X(033)         VALUE
               'TOT DE ERROS NO GRUP201C       : '.
           03  LT14-TOT-ERROS-GRUP201C PIC  ZZZ.ZZZ.ZZZ.ZZZ.ZZ9.

       01  LINTOT15.
           03  FILLER                  PIC  X(001)         VALUE '0'.
           03  FILLER                  PIC  X(010)         VALUE SPACES.
           03  FILLER                  PIC  X(033)         VALUE
               'TOT DE ENCONTRADOS NO GRUP201C : '.
           03  LT15-TOT-ENC-GRUP201C   PIC  ZZZ.ZZZ.ZZZ.ZZZ.ZZ9.

       01  LINTOT16.
           03  FILLER                  PIC  X(001)         VALUE '0'.
           03  FILLER                  PIC  X(010)         VALUE SPACES.
           03  FILLER                  PIC  X(033)         VALUE
               'TOT DE NAO ENCONTR NO GRUP201C : '.
           03  LT16-TOT-N-ENC-GRUP201C PIC  ZZZ.ZZZ.ZZZ.ZZZ.ZZ9.
...
-INC POL7100C
      ...
      *----------------------------------------------------------------*
       01  FILLER                      PIC  X(050)         VALUE
           '*** SLIG6215 - FIM DA AREA DE WORKING ***'.
      *----------------------------------------------------------------*
...
VOLTAR*        P*RFORM 999999-PROCESSAR-ROTINA-ERRO
      ...
      *----------------------------------------------------------------*
           ...
           MOVE SLIG74-BCO-CLI         TO WRK-BCO-ATU
           MOVE SLIG74-AGENCIA-CLI     TO WRK-AGE-ATU
           MOVE SLIG74-CONTA-CLI       TO WRK-CTA-ATU
      ...
      *--------  FORMATA GRUPO ECONOMICO ATRAVES DO GRUP201C  ---------*
      ...
      *--  FORMATA CAMPOS DO CLIENTE PARA GRAVACAO  -------------------*
           ...
           MOVE WRK-GRP-ECONOMICO-CLI    TO SLIG74-GRP-ECONOMICO-CLI.
           MOVE WRK-CGC-CPF-CLI          TO SLIG74-CGC-CPF-CLI-ST.
           MOVE WRK-FILIAL-CLI           TO SLIG74-FILIAL-CLI-ST.
           MOVE WRK-CONTROLE-CLI         TO SLIG74-CONTROLE-CLI-ST.
           MOVE WRK-NOME-CLI             TO SLIG74-NOME-CLI.
           MOVE WRK-BCO-DIR-REGIONAL     TO SLIG74-BCO-DIR-REGIONAL.
           MOVE WRK-DIR-REGIONAL         TO SLIG74-DIR-REGIONAL.
           MOVE WRK-BCO-GERC-REGIONAL    TO SLIG74-BCO-GERC-REGIONAL.
           MOVE WRK-GERC-REGIONAL        TO SLIG74-GERC-REGIONAL.

      *--  FORMATA NOME EMITENTE CASO CNPJ/CPF SEJA IGUAL AO CLIENTE --*

           IF SLIG74-CGC-CPF-CLI-ST     EQUAL SLIG74-CGC-CPF-EMITT-ST
           AND
              SLIG74-FILIAL-CLI-ST      EQUAL SLIG74-FILIAL-EMITT-ST
              AND
              SLIG74-CONTROLE-CLI-ST    EQUAL SLIG74-CONTROLE-EMITT-ST
              AND
              SLIG74-NOME-CLI        NOT EQUAL SPACES             AND
              SLIG74-NOME-CLI        NOT EQUAL
                                     'CLIENTE NAO ENCONTRADO'     AND
              SLIG74-NOME-EMITT      NOT EQUAL SLIG74-NOME-CLI
              MOVE SLIG74-NOME-CLI       TO SLIG74-NOME-EMITT
           END-IF.

      *--  FORMATA TIPO DE PESSOA CLIENTE  ----------------------------*

           IF SLIG74-CGC-CPF-CLI-ST
           NOT EQUAL ZEROS AND
              SLIG74-FILIAL-CLI-ST
              EQUAL ZEROS
              MOVE 'F'                   TO SLIG74-TIPO-PESSOA-CLI
           ELSE
              MOVE 'J'                   TO SLIG74-TIPO-PESSOA-CLI
           END-IF.

      *--  ACUMULA GERENCIA REGIONAL CLIENTE ZERADA  ------------------*

           IF SLIG74-BCO-GERC-REGIONAL EQUAL ZEROS AND
              SLIG74-GERC-REGIONAL     EQUAL ZEROS
      ...
      *--  ACUMULA DIRETORIA REGIONAL CLIENTE ZERADA  -----------------*
           ...
           IF SLIG74-BCO-DIR-REGIONAL  EQUAL ZEROS AND
              SLIG74-DIR-REGIONAL      EQUAL ZEROS
      ...
      *--  ACUMULA GRUPO ECONOMICO CLIENTE ZERADO  --------------------*
           ...
           IF SLIG74-GRP-ECONOMICO-CLI EQUAL ZEROS
      ...
      *--  ACUMULA NOME CLIENETE NAO ECONTRADO E SPACES ---------------*
           ...
           IF SLIG74-NOME-CLI         EQUAL 'CLIENTE NAO ENCONTRADO'
              ADD 1                      TO ACU-CLI-NOME-N-ENC-OUT
           ELSE
              IF SLIG74-NOME-CLI      EQUAL SPACES
      ...
      *--  GRAVA REGISTRO EM EMITCHEQ  --------------------------------*
           ...
           WRITE REG-CHEQUE-SAIDA  FROM  REG-CHEQUE-EMITENTE-RELATO.
      ...
      *----------------------------------------------------------------*
           ...
           MOVE SLIG74-BCO-CLI         TO COMU-COD-EMPR.
           MOVE SLIG74-AGENCIA-CLI     TO COMU-COD-DEPDC.
           ...
           IF RETURN-CODE              EQUAL ZEROS
              ...
              MOVE SLIG74-BCO-CLI TO WRK-BCO-DIR-REGIONAL
      ...
      *----------------------------------------------------------------*
           ...
           MOVE 'SLIG6215'             TO WRK-9023-NOME-PGM.
           MOVE 'B'                    TO WRK-9023-AMBIENTE.

           MOVE SLIG74-BCO-CLI         TO WRK-9023-BANCO.
           MOVE SLIG74-AGENCIA-CLI     TO WRK-9023-AGENCIA.
      ...
      *----------------------------------------------------------------*
           ...
           PERFORM 325100-ACESSAR-GRUP201C.

           IF  GRUPW01S-RETURN-CODE EQUAL ZEROS
               ADD  1                     TO ACU-GRUP201C-GRP-ENC
           ...
           ELSE
               ...
               IF  GRUPW01S-RETURN-CODE NOT EQUAL 1
                   ADD  1                 TO ACU-GRUP201C-ERRO

                   IF ACU-GRUP201C-ERRO NOT GREATER 100

                      IF GRUPW01S-RETURN-CODE EQUAL 4
                         MOVE 'ARGUMENTO DE PESQUISA INCONSISTENTE'
                                         TO WRK-GRUP201C-MSG-ERRO
                      ELSE
                         MOVE 'ERRO DB2' TO WRK-GRUP201C-MSG-ERRO
                      END-IF
                      ...
                      DISPLAY '%% GRUP201C '
                              ...
                              '-' WRK-GRUP201C-MSG-ERRO

                   END-IF
               ELSE
                   ADD  1                 TO ACU-GRUP201C-GRP-N-ENC
               END-IF
           END-IF.
      ...
      *----------------------------------------------------------------*
           EJECT
      *----------------------------------------------------------------*
       325100-ACESSAR-GRUP201C         SECTION.
      *----------------------------------------------------------------*

           MOVE 'SLIG6215'             TO GRUPW01S-PGM-CHAMADOR.
      ...
      *----------------------------------------------------------------*
           ...
           MOVE ACU-ACESSOS-GRUP201C    TO LT13-TOT-ACESSO-GRUP201C.
           MOVE ACU-GRUP201C-ERRO       TO LT14-TOT-ERROS-GRUP201C.
           MOVE ACU-GRUP201C-GRP-ENC    TO LT15-TOT-ENC-GRUP201C.
           MOVE ACU-GRUP201C-GRP-N-ENC  TO LT16-TOT-N-ENC-GRUP201C.
