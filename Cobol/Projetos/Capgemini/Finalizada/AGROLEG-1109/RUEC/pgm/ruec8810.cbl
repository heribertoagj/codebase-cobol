...
CCM01A 77  WRK-IND-B045            PIC S9(05) COMP-3 VALUE ZEROS.
...
CCM01A 77  WRK-CODE100-B045        PIC  X(01) VALUE 'N'.
...
CCM01A 01  WRK-BASE-LEGAL          PIC X(05)   VALUE SPACES.
CCM01A 01  FILLER REDEFINES WRK-BASE-LEGAL.
CCM01A     05 FILLER               PIC  X(02).
CCM01A     05 WRK-BASE-LEGAL-RENG  PIC  X(03).
...
BS0216     END-EXEC.
BS0216
CCM01A     EXEC SQL   INCLUDE RUFIB045   END-EXEC.
...
CCM01A*----------------------------------------------------------------*
       01  FILLER                      PIC X(50)           VALUE
           '*  DEFINICAO CURSOR CSR01-RUFIB045    *'.
      *----------------------------------------------------------------*

           EXEC SQL
           DECLARE CSR01-RUFIB045 CURSOR  WITH HOLD  FOR
           SELECT  CCONTR
                  ,CREFT_RECOR
                  ,VRENEG_CONTR_RURAL
                  ,CBASE_LEGAL_RENEG
...
AQUI              PERFORM 3000-OBTEM-DADOS-B099
...
AC0513     COMPUTE WRK-POSICAO2 = WRK-TAM2 + WRK-1.
      ...
      * FORMATA GRUPO DESTCFIN
           MOVE 1     TO WRK-IND1.
AQUI       PERFORM 3120-FORMATA-GRUPO-DESTCFIN
...
CCM01A     MOVE 'N' TO WRK-CODE100-B045.
           MOVE  1  TO WRK-IND2.
...
CCM01A     PERFORM 3401-OPEN-CSR01-RUFIB045.
CCM01A     PERFORM 3402-SELECT-RUFIB045.

CCM01A     IF WRK-CODE100-B045 EQUAL 'S'
...
AC0513        ADD LENGTH OF RUEC66-COR0001-4 TO WRK-POSICAO2
           ELSE
CCM01A        PERFORM VARYING  WRK-IND-B045     FROM 1 BY 1
.                  UNTIL WRK-CODE100-B045  EQUAL 'S'
.                  PERFORM 3404-FORMATAR-ITEMB045
.                  PERFORM 3402-SELECT-RUFIB045
.             END-PERFORM
.
.             MOVE 'N'   TO WRK-CODE100-B045
.             MOVE ZEROS TO WRK-IND-B045
.             ADD  999   TO WRK-IND2
.
.             MOVE ALL '*' TO RUEC66-GRUPO-COR0001-RENEGC
.             MOVE RUEC66-COR0001-4  TO
.                WRK-B09A-SIS2(WRK-POSICAO2:
.                             LENGTH OF RUEC66-COR0001-4)
.             ADD LENGTH OF RUEC66-COR0001-4 TO WRK-TAM2
.             ADD LENGTH OF RUEC66-COR0001-4 TO WRK-POSICAO2
.          END-IF.
.
CCM01A     PERFORM 3403-CLOSE-CSR01-RUFIB045.
...
BS0216*----------------------------------------------------------------*
.     *---------------------------------------------------------------*
CCM01A 3401-OPEN-CSR01-RUFIB045    SECTION.
.     *---------------------------------------------------------------*
.
.          MOVE RUEC65-CCONTR  TO CCONTR  OF RUFIB045
.
.          EXEC SQL  OPEN CSR01-RUFIB045  END-EXEC.
.
.          IF ( SQLCODE   NOT  EQUAL  ZEROS )   OR
.             ( SQLWARN0       EQUAL  'W'   )
.             MOVE 99                   TO RUEC65-COD-RET
.             MOVE 'ERRO ACESSO DB2'    TO RUEC65-MENSAGEM
.             MOVE 'DB2'                TO ERR-TIPO-ACESSO
.             MOVE 'TCONTR_RURAL_RENEG' TO ERR-DBD-TAB
.             MOVE 'OPEN    '           TO ERR-FUN-COMANDO
.             MOVE SQLCODE              TO ERR-SQL-CODE
...
.      3401-99-FIM.                    EXIT.
.     *----------------------------------------------------------------*
.      3402-SELECT-RUFIB045 SECTION.
.     *---------------------------------------------------------------*
.
.          EXEC SQL
.             FETCH CSR01-RUFIB045 INTO
.                :RUFIB045.CCONTR,
.                :RUFIB045.CREFT-RECOR,
...
.     *----------------------------------------------------------------*
.     *----------------------------------------------------------------*
.      3403-CLOSE-CSR01-RUFIB045    SECTION.
.     *---------------------------------------------------------------*
.
.          EXEC SQL   CLOSE CSR01-RUFIB045  END-EXEC.
.
.          IF ( SQLCODE   NOT  EQUAL    ZEROS )   OR
.             ( SQLWARN0       EQUAL    'W'   )
.              MOVE 99                   TO RUEC65-COD-RET
...
.     *----------------------------------------------------------------*
.     *---------------------------------------------------------------*
.      3404-FORMATAR-ITEMB045    SECTION.
.     *---------------------------------------------------------------*
.
.          IF SQLCODE EQUAL +100
.             GO TO 3404-99-FIM
...
.                                      TO RUEC66-NUMREF-BCCOR-RENEGC
.             MOVE VRENEG-CONTR-RURAL  OF RUFIB045 TO WRK-VLR15
.             MOVE WRK-VLR15-CHAR      TO WRK-VLR15-PARC
.             MOVE WRK-VLR1            TO RUEC66-VLR-RENEGC
.             MOVE CBASE-LEGAL-RENEG   OF RUFIB045 TO WRK-BASE-LEGAL
.             MOVE WRK-BASE-LEGAL-RENG TO RUEC66-BASE-LEGAL-RENEGC
.             MOVE RUEC66-COR0001-4    TO
.                  WRK-B09A-SIS(WRK-POSICAO:
.                                      LENGTH OF RUEC66-COR0001-4)
.             ADD LENGTH OF RUEC66-COR0001-4  TO WRK-TAM
