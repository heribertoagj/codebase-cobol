      ...
      *----------------------------------------------------------------*
      *    BOOK'S......:                                               *
      *    I#RUEC65 - AREA DE RETORNO DO MODULO RUEC8830               *
...
7C1019*---------------------------------------------------------------*
7C1019*                                                               *
7C1019*        ANALISTA : EDSON MATSUMOTO - 7COMM                     *
7C1019*        DATA     : OUTUBRO / 2019                              *
7C1019*        OBJETIVO : IN3373357 - TRATAMENTO QUANDO A ORIGEM DE   *
7C1019*                   RECURSOS NAO TEM ASSOCIADO O CODIGO DE O    *
7C1019*                   PLANO REFIN, GERA * NO CAMPO TpFnteRec DA   *
7C1019*                   COR0005, POIS É UM CAMPO NAO OBRIGATORIO    *
      *================================================================*
WMM   *                    ALTERACAO - PRIME                           *
WMM   *----------------------------------------------------------------*
...
qt1120*01  FILLER.
qt1120*    03 WRK-SZD17V2-AUX          PIC S9(17)V99 VALUE ZEROS.
qt1120*    03 FILLER                   REDEFINES WRK-SZD17V2-AUX.
qt1120*       05  FILLER               PIC  X(14).
qt1120*       05  WRK-SZD05            PIC S9(05).
...
BR4.11     '*** AREA DO MODULO RUEC7941  ***'.
BR4.11*----------------------------------------------------------------*
BR4.11
...
RE1117*---------------------------------------------------------------*
RE1117 01  FILLER                      PIC  X(32)          VALUE
RE1117     '**** INCLUDE RUFIB093        ***'.
RE1117*---------------------------------------------------------------*
RE1117
RE1117     EXEC SQL
RE1117         INCLUDE RUFIB093
...
BR0621     05 WRK-B002-PENERG-NULL     PIC S9(04) COMP VALUE +0.
       ...
       01  WRK-B093-NULL.
           05 WRK-B093-NBASE-NULL  PIC S9(04) COMP VALUE +0.
...
BR1118     END-IF.
      ...
      ***  CodBaseLegalProrrogc
           MOVE RUEC65-CCONTR          TO CCONTR         OF RUFIB093
           PERFORM 3141-OBTER-MOTIVO-PORROG

           IF SQLCODE                  EQUAL +100
              MOVE ALL '*'             TO RUECC5-CBASE-LEGAL-PRO
           ELSE
              MOVE DRENEG-CONTR-RURAL  OF RUFIB093
                                       TO WRK-DATA-DB2
              MOVE WRK-DIA-DB2         TO WRK-DIA-ISO
              MOVE WRK-MES-DB2         TO WRK-MES-ISO
              MOVE WRK-ANO-DB2         TO WRK-ANO-ISO
...
BR0521***  Grupo_COR0005_Desclass

      ***  Grupo_COR0005_Cooperd
           PERFORM 3170-TRATAR-GRUPO-COOPERADO.

4.13  ***  ORIGEM DE RECURSO
4.13       MOVE CORIGE-REC OF RUFIB002  TO CORIGE-REC OF RURCB000
4.13       PERFORM 4500-SELECT-RURCB000
7C1019     IF CPLANO-REFIN-RECOR OF RURCB028 EQUAL SPACES
7C1019         MOVE  ALL '*'            TO RUECC5-TP-FNTE-REC
7C1019     ELSE
...
MJ1224**********OVE CMRUPP-S-TAM-GRUPO TO WRK-TAM-PPRIE-PD
MJ1224**********OVE CMRUPP-S-GRUPO-PROP(1:WRK-TAM-PPRIE-PD  )
MJ1224**********                       TO WRK-B09A-SIS(WRK-POSICAO:
MJ1224**********                          WRK-TAM-PPRIE-PD  )
MJ1224**********DD WRK-TAM-PPRIE-PD    TO WRK-TAM
MJ1224**********DD WRK-TAM-PPRIE-PD    TO WRK-POSICAO
...
.          END-IF
.
.          IF  RUECC5-COD-PROGRAMA     EQUAL SPACES AND
...
BR1118*----------------------------------------------------------------*
      ...
      *---------------------------------------------------------------*
           ...
           IF  SQLCODE                 EQUAL +100
           OR  NENVIO-RECOR            OF RUECB09B
                                       EQUAL ZEROS
BR1118         PERFORM 3105-INSERE-HIST
**************-MOVE 04                 TO RUEC65-COD-RET
**************-MOVE 'CONTRATO NAO FOI ENVIADO AO BACEN'
**************-                        TO RUEC65-MENSAGEM
**************-PERFORM 9000-ROTINA-FIM
           ELSE
...
BR0621     END-IF.
      ...
      *---------------------------------------------------------------*
       3141-OBTER-MOTIVO-PORROG        SECTION.
      *---------------------------------------------------------------*

           EXEC SQL
             SELECT NBASE_LEGAL_RENEG
             ,      NRENEG_CONTR_RURAL
             ,      DRENEG_CONTR_RURAL
             INTO   :RUFIB093.NBASE-LEGAL-RENEG
                    :WRK-B093-NBASE-NULL
           ...
           OR  (SQLWARN0               EQUAL 'W')
               ...
               MOVE 'SELECT'           TO ERR-FUN-COMANDO
               MOVE SQLCODE            TO ERR-SQL-CODE
               MOVE '3141'             TO ERR-LOCAL
               MOVE SPACES             TO ERR-SEGM
               PERFORM 9000-ROTINA-FIM
           END-IF.

           IF WRK-B093-NBASE-NULL  NOT EQUAL ZEROS
                MOVE ZEROS   TO NBASE-LEGAL-RENEG  OF RUFIB093
           END-IF

           IF SQLCODE  NOT EQUAL +100
              PERFORM 3142-OBTER-COD-BACEN
           END-IF.
...
-     *---------------------------------------------------------------*
-
-          MOVE RUEC65-CCONTR          TO CCONTR OF RUFIB044.
-
-          PERFORM 3161-OPEN-CSR02-RUFIB044-JN.
-
-          PERFORM 3162-FETCH-CSR02-RUFIB044-JN.
-
-***********F  WRK-FIM-CSR02           EQUAL 'S'
-     ***      DtDesclassEmpnmnt
-              MOVE ALL '*'            TO RUECC5-DT-DESCLASS
-     ***      MotvDesclassCOR
-              MOVE ALL '*'            TO RUECC5-MTV-DESCLASS
...
-***********ND-IF
-
-          PERFORM 3164-MONTAR-DESCLASSIFICACOES
-*************NTIL WRK-FIM-CSR02       EQUAL 'S'.
-
-          PERFORM 3163-CLOSE-CSR02-RUFIB044-JN.
-
-     *----------------------------------------------------------------*
-      3160-99-FIM.                    EXIT.
-     *----------------------------------------------------------------*
...
-     *---------------------------------------------------------------*
-     ***  PRECISOU PULAR POR CAUSA DE APONTAMENTO HEXA.
-          GO   TO                     3164-99-FIM.
-
-     ***  DtDesclassEmpnmnt
...
WIP001*----------------------------------------------------------------*
BR0518*---------------------------------------------------------------*
BR0518 3170-TRATAR-GRUPO-COOPERADO     SECTION.
BR0518*---------------------------------------------------------------*
...
BR0518         MOVE ALL '*'            TO RUECC5-TPSSOA-COOP
BR0518
BR0518***      CNPJ_CPFCooperd
BR0518         MOVE ALL '*'            TO RUECC5-CNPJ-CPF-COOP-X
BR0518
BR0518***      VlrParclCooperd
BR0518         MOVE ALL '*'            TO RUECC5-VLR-PCELA-COOP
BR0518
BR0518***      CodProg_LinhaCredCooperd
BR0518         MOVE ALL '*'            TO RUECC5-CODPROG-LINCRED
BR0518
...
211025**   CAMPOS EXCLUIDOS - SICOR 5.11
STE001*---------------------------------------------------------------*
STE001*3183-TRATAR-GLEBA-EXCLUIDA    SECTION.
STE001*---------------------------------------------------------------*
STE001*
...
4.13       END-EXEC.
4.13
7C1019     IF (SQLCODE                  NOT EQUAL ZEROS AND +100) OR
4.13          (SQLWARN0                 EQUAL 'W')
4.13           MOVE 99                  TO RUEC65-COD-RET
4.13           MOVE 'ERRO ACESSO DB2'   TO RUEC65-MENSAGEM
4.13           MOVE 'DB2'               TO ERR-TIPO-ACESSO
...
4.13       END-IF.
4.13
7C1019     IF SQLCODE  EQUAL +100
7C1019        MOVE SPACES TO CPLANO-REFIN-RECOR OF RURCB028
7C1019     END-IF.
4.13  *---------------------------------------------------------------*
...
BR0621*---------------------------------------------------------------*
BR0621
MJ1224*    NAO MONTAR O GRP DE PROPRIETARIOS NA COR0005
...
BR0621                                OF RUECB09E.
BR0621     MOVE RUEC65-NENVIO-RECOR   TO NENVIO-RECOR
BR0621                                OF RUECB09E.
BR0621
BR0621     EXEC SQL
...
BR0621        ELSE
STEFA1           MOVE WRK-CNIRF-IMOV(WRK-IND-B09E)(2:8)
