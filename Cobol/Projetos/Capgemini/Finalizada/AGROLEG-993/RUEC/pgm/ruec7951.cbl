      ...
      *----------------------------------------------------------------*
       ...
       01 WRK-INDEXCL                   PIC  9(02)  COMP-3 VALUE ZEROS.
       ...
       01 WRK-INDCD-DEL                 PIC  X(01)         VALUE SPACES.
       ...
       01 IND-1                         PIC  9(003) COMP-3
                                                       VALUE ZEROS.
       01 WRK-FIM-CSR01                 PIC  X(001)    VALUE SPACES.
       01 WRK-FIM-CSR02                 PIC  X(001)    VALUE SPACES.
      ...
      *----------------------------------------------------------------*
       ...
       01  WRK-660-RUECT797.
           ...
           03 WRK-660-INDEXCL            PIC S9(004) COMP  VALUE 0002.
           ...
           03 WRK-660-LISTA              OCCURS 7.
              05 WRK-660-ITEMLS          PIC S9(004) COMP  VALUE 2003.
...
*************** B=BATCH / OUTROS=IMS ONLINE
      ...
      *----------------------------------------------------------------*
           ...
           PERFORM                     VARYING IND-1
              ...
              MOVE SPACES              TO WRK-O-ITEMLS(IND-1)
                                          ...
                                          WRK-I-ITEMLS(IND-1)
                                          WRK-I-REFBACEN(IND-1)

              MOVE SPACES              TO WRK-O-CODBAS(IND-1)
                                          WRK-O-CHVOPE(IND-1)
                                          WRK-O-SLDPRI(IND-1)
                                          WRK-O-SLDENC(IND-1)
                                          WRK-O-SLDREN(IND-1)
      ...
      *----------------------------------------------------------------*
           ...
           PERFORM 1350-OBTER-VALOR-TOTAL
      ...
      *----------------------------------------------------------------*
      *----------------------------------------------------------------*
       1350-OBTER-VALOR-TOTAL          SECTION.
      *----------------------------------------------------------------*

           EXEC SQL
              SELECT SUM(VRENEG_CONTR_RURAL)
                INTO :RUECB00C.VRENEG-CONTR-RURAL
                FROM DB2PRD.TCDULA_TEMPR_RENEG
               WHERE CJUNC_DEPDC = :RUECB00C.CJUNC-DEPDC
                 AND CCDULA = :RUECB00C.CCDULA
           END-EXEC.
      ...
      *  DA REFERENCIA BACEN
           ...
           IF WRK-I-FASE               EQUAL 4 OR 5
              PERFORM 1210-MONTAR-TELA
              MOVE 'TECLE PF6 PARA INSERIR OU <ENTER> CANCELAR'
                                    TO WRK-O-MENSA
              GO                    TO 3100-99-FIM
           END-IF

           IF WRK-INDCD-DEL        EQUAL 'S'
              MOVE 6                TO WRK-I-FASE
                                       WRK-O-FASE

              MOVE WRK-I-INDEXCL    TO WRK-INDEXCL
              MOVE WRK-225          TO WRK-O-ITEMLS-ATTR
                                       (WRK-INDEXCL)

              STRING 'TECLE PF6 PARA CONFIRMAR EXCLUSAO '
                     'OU <ENTER> PARA CANCELAR'
                     DELIMITED BY SIZE INTO WRK-O-MENSA
              GO                    TO 3100-99-FIM
           END-IF

           IF WRK-INDCD-INCL        EQUAL 'N'
      ...
      *----------------------------------------------------------------*
           ...
           MOVE WRK-I-INDEXCL          TO WRK-O-INDEXCL.
           MOVE WRK-I-FASE             TO WRK-O-FASE.
      ...
      *----------------------------------------------------------------*
           IF WRK-I-FASE               NOT EQUAL ZEROS
              MOVE SPACES              TO WRK-AUX-MENSA
                                          WRK-O-MENSA
      ...
      *----------------------------------------------------------------*
           ...
           MOVE WRK-I-CEDULA           TO CCDULA
                                       OF RUECB00D
           MOVE WRK-I-INDEXCL          TO WRK-INDEXCL
           MOVE WRK-I-REFBACEN(WRK-INDEXCL)
                                       TO CREFT-RECOR
                                       OF RUECB00D
           EXEC SQL
              OPEN CSR02-RUECB00D
      ...
      *----------------------------------------------------------------*
           ...
           EXEC SQL
              DELETE FROM DB2PRD.TCDULA_TEMPR_RENEG
              WHERE CJUNC_DEPDC = :RUECB00C.CJUNC-DEPDC
                AND CCDULA      = :RUECB00C.CCDULA
      ...
      *----------------------------------------------------------------*
           ...
           AND WRK-QTD-DEL             EQUAL ZEROS
              MOVE 'S'                 TO WRK-INCONSIS
              STRING 'INFORME REFBACEN OU SELECIONE ITEM PARA'
                     ' CONSULTA, <X> OU <E> PARA EXCLUSAO.'
                     DELIMITED BY SIZE INTO WRK-AUX-MENSA
              PERFORM 4400-MOVER-MENSAGEM
           ...
           IF  WRK-QTD-SEL             GREATER ZEROS
           AND WRK-QTD-DEL             GREATER ZEROS
              MOVE 'S'                 TO WRK-INCONSIS
              STRING 'ESCOLHA APENAS UMA ACAO: CONSULTA OU'
                     ' EXCLUSAO (<X> OU <E>).'
                     DELIMITED BY SIZE INTO WRK-AUX-MENSA
              PERFORM 4400-MOVER-MENSAGEM
              MOVE WRK-49353           TO WRK-O-ITEMLS-ATTR
           ...
           IF WRK-QTD-DEL              GREATER 1
              MOVE 'S'                 TO WRK-INCONSIS
              MOVE 'SELECIONE APENAS UM ITEM PARA EXCLUSAO.'
                                       TO WRK-AUX-MENSA
              PERFORM 4400-MOVER-MENSAGEM
              MOVE WRK-49353           TO WRK-O-ITEMLS-ATTR
                                          (WRK-IND-SEL)
           ...
           IF WRK-QTD-SEL              GREATER 1
              MOVE 'S'                 TO WRK-INCONSIS
              MOVE 'SELECIONE APENAS UM ITEM PARA CONSULTA.'
                                       TO WRK-AUX-MENSA
              PERFORM 4400-MOVER-MENSAGEM
              MOVE WRK-49353           TO WRK-O-ITEMLS-ATTR
                                          (WRK-IND-SEL)
           ...
           IF WRK-QTD-DEL              EQUAL 1
              MOVE 'S'                 TO WRK-INDCD-DEL
              MOVE WRK-IND-DEL         TO WRK-I-INDEXCL
                                          WRK-O-INDEXCL
           END-IF.
      ...
      *----------------------------------------------------------------*
           ...
           IF WRK-W602-S-SLD-TOTAL     NOT NUMERIC
           OR WRK-W602-S-SLD-TOTAL     EQUAL ZEROS
              MOVE 'S'                 TO WRK-INCONSIS
              STRING 'NAO EH PERMITIDO REORGANIZAR CONTRATO SEM'
                     ' PARCELA EM ATRASO OU A VENCER'
                     DELIMITED BY SIZE INTO WRK-AUX-MENSA
              PERFORM 4400-MOVER-MENSAGEM
              GO TO 4200-99-FIM
           END-IF.
