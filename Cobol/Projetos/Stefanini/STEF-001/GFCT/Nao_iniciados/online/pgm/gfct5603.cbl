      ...
      *                                                                *
      *----------------------------------------------------------------*
      *    OBJETIVO....:   COMBO DE TARIFAS CONFIGURACAO DE PACOTES -  *
      *      ADESAO ABERTURA CONTAS CORRESP. BANCARIO.                 *
      *                                                                *
      *----------------------------------------------------------------*
      *    BANCO DE DADOS:                                             *
      *      DB2                                                       *
...
BI0115*                   ( PACOTE PERSONALIZADO )                     *
      ...
      *----------------------------------------------------------------*
       ...
       01  WRK-QTDE-TOT-REG            PIC 9(06) COMP-3    VALUE ZEROS.
       01  WRK-QTDE-OCOR               PIC 9(03) COMP-3    VALUE ZEROS.
       ...
       01  WRK-CPSSOA-TARIF-INIC       PIC X(01)           VALUE SPACES.
       01  WRK-CPSSOA-TARIF-FIM        PIC X(01)           VALUE SPACES.
       01  WRK-MONTA                   PIC X(01)           VALUE SPACES.
...
BI0115 01  WRK-CTPO-SERVC-TARIF-4      PIC S9(001) COMP-3   VALUE +4.
       ...
       01  WRK-PSSOA-X                 PIC  X(001)         VALUE ZEROS.
       01  FILLER                      REDEFINES           WRK-PSSOA-X.
           05  WRK-PSSOA               PIC  9(001).
...
BI0115              OR CTPO_SERVC_TARIF   = :WRK-CTPO-SERVC-TARIF-4)
               AND    (CPSSOA_SERVC_TARIF = '3'
               OR     (CPSSOA_SERVC_TARIF >= :WRK-CPSSOA-TARIF-INIC
               AND     CPSSOA_SERVC_TARIF <= :WRK-CPSSOA-TARIF-FIM ) )
      ...
      *================================================================*
       PROCEDURE DIVISION              USING WAHC-ENTRADA
                                             WAHD-SAIDA
                                             GFCT0M-AREA-ERROS.
      *================================================================*
      ...
      *----------------------------------------------------------------*
           ...
           INITIALIZE WAHD-SAIDA.

           MOVE +261                   TO WAHD-LL.
           MOVE ZEROS                  TO WAHD-ZZ.
           MOVE WAHC-TRANSACAO         TO WAHD-TRANSACAO.
           MOVE WAHC-FUNCAO            TO WAHD-FUNCAO.

           IF  WAHC-QTDE-OCOR        NOT NUMERIC
               MOVE ZEROS              TO WAHD-QTDE-OCOR
           ELSE
               MOVE WAHC-QTDE-OCOR     TO WAHD-QTDE-OCOR
                                          WRK-QTDE-OCOR
           END-IF.

           MOVE ZEROS                  TO WAHD-QTDE-TOT-REG
                                          WRK-QTDE-TOT-REG

           MOVE WAHC-FUNC-BDSCO        TO WAHD-FUNC-BDSCO.
           MOVE WAHC-TERMINAL          TO WAHD-TERMINAL.
           MOVE WAHC-FILTRO            TO WAHD-FILTRO
           MOVE WAHC-PONTEIRO-TARIFA
                                       TO WAHD-PONTEIRO-TARIFA.
           MOVE WAHC-PONTEIRO-DATA     TO WAHD-PONTEIRO-DATA.

           MOVE 'N'                    TO WAHD-FIM.

           MOVE ZEROS                  TO WAHD-ERRO
                                          WAHD-COD-SQL-ERRO
                                          WAHD-COD-MSG-ERRO
                                          GFCTG2-COD-MSG.
           ...
           MOVE GFCTG3-DESC-MSG        TO WAHD-DESC-MSG-ERRO.
      ...
      *----------------------------------------------------------------*
           ...
           MOVE WAHC-TRANSACAO         TO GFCTG2-TRANSACAO.
           MOVE WAHC-FUNCAO            TO GFCTG2-FUNCAO.
           MOVE WAHC-FUNC-BDSCO        TO GFCTG2-FUNC-BDSCO.
           ...
           IF  GFCTG3-ERRO                 EQUAL 1
               ...
               MOVE GFCTG3-COD-SQL-ERRO
                                       TO WAHD-COD-SQL-ERRO
               MOVE GFCTG3-COD-MSG-ERRO
                                       TO WAHD-COD-MSG-ERRO
               MOVE GFCTG3-DESC-MSG-ERRO
                                       TO WAHD-DESC-MSG-ERRO
               PERFORM 3000-FINALIZAR
           END-IF.
      ...
      *----------------------------------------------------------------*
           ...
           IF (WAHC-TRANSACAO        EQUAL SPACES)          OR
              (WAHC-FUNCAO           EQUAL SPACES)          OR
              (WAHC-FTR-TARIF        IS NOT NUMERIC)              OR
              (WAHC-QTDE-OCOR        IS NOT NUMERIC)              OR
              (WAHC-QTDE-TOT-REG     IS NOT NUMERIC)              OR
              (WAHC-FUNC-BDSCO       EQUAL SPACES)                OR
              (WAHC-PONTEIRO-TARIFA  IS NOT NUMERIC)              OR
              (WAHC-TIPO-DATA        EQUAL  0                     AND
               WAHC-PONTEIRO-DATA    EQUAL SPACES)                OR
              (WAHC-FIM              NOT EQUAL 'S' AND 'N')       OR
              (WAHC-TIPO-VIGENCIA    NOT EQUAL 'V' AND 'N'        AND
                                                 'T' AND 'P'      AND
                                                 'H' AND 'F')     OR
              (WAHC-TIPO-DATA        IS NOT NUMERIC               OR
               WAHC-TIPO-DATA        NOT EQUAL  0  AND  1         AND
                                                  2  AND  3 )     OR
              (WAHC-FTR-TP-PSSOA       IS NOT NUMERIC             OR
               WAHC-FTR-TP-PSSOA       NOT EQUAL  0 AND 1 AND 2   AND
                                                  3 AND 4 AND 5)
               MOVE 1                  TO WAHD-ERRO
               MOVE ZEROS              TO WAHD-COD-SQL-ERRO
               MOVE 0001               TO WAHD-COD-MSG-ERRO
      ...
      *----------------------------------------------------------------*
           ...
           MOVE WAHC-TRANSACAO         TO GFCTFZ-TRANSACAO.
           MOVE WAHC-FUNCAO            TO GFCTFZ-FUNCAO.
           MOVE WAHC-FUNC-BDSCO        TO GFCTFZ-FUNC-BDSCO.
           MOVE WAHC-FIM               TO GFCTFZ-FIM.
      ...
      *----------------------------------------------------------------*
           ...
           IF  WAHC-PONTEIRO-TARIFA  EQUAL ZEROS
               MOVE WAHC-FTR-TARIF     TO WRK-AUX-TARIFA
               MOVE '01.01.0001'       TO WRK-DATA-PONTEIRO
           ELSE
               MOVE WAHC-PONTEIRO-TARIFA
                                       TO WRK-AUX-TARIFA
               MOVE WAHC-PONTEIRO-DATA
                                       TO WRK-DATA-PONTEIRO
           END-IF.

           IF  WAHC-FTR-TP-PSSOA    EQUAL 0 OR 3
               MOVE  '1'               TO WRK-CPSSOA-TARIF-INIC
               MOVE  '2'               TO WRK-CPSSOA-TARIF-FIM
           ELSE
            IF  WAHC-FTR-TP-PSSOA   EQUAL 2 OR 5
                MOVE  '2'              TO WRK-CPSSOA-TARIF-INIC
                MOVE  '2'              TO WRK-CPSSOA-TARIF-FIM
            ELSE
                MOVE  '1'              TO WRK-CPSSOA-TARIF-INIC
                MOVE  '1'              TO WRK-CPSSOA-TARIF-FIM
            END-IF
           END-IF.

           EVALUATE WAHC-TIPO-VIGENCIA
               WHEN 'T'
...
ST25X6*    Ajuste para campos alfanuméricos do CNPJ
           ...
           MOVE WAHC-FUNC-BDSCO        TO WRK-COD-FUNCIONAL-0315.
           ...
           IF  RETURN-CODE            NOT EQUAL ZEROS AND 04
               MOVE 'S'                TO WAHD-FIM
               MOVE 1                  TO WAHD-ERRO
               MOVE RETURN-CODE        TO WAHD-COD-SQL-ERRO
               MOVE 0056               TO WAHD-COD-MSG-ERRO
           ...
           IF  RETURN-CODE          EQUAL 04
               ...
               IF  WRK-COD-MENSAGEM-0315   EQUAL 001
                   MOVE 1              TO WAHD-ERRO
                   MOVE ZEROS          TO WAHD-COD-SQL-ERRO
                   MOVE 0057           TO WAHD-COD-MSG-ERRO
               ...
               IF  WRK-COD-MENSAGEM-0315   EQUAL 154
                   MOVE 1              TO WAHD-ERRO
                   MOVE ZEROS          TO WAHD-COD-SQL-ERRO
                   MOVE 0058           TO WAHD-COD-MSG-ERRO
               ...
               IF  WRK-COD-MENSAGEM-0315   EQUAL 242
                   MOVE 1              TO WAHD-ERRO
                   MOVE ZEROS          TO WAHD-COD-SQL-ERRO
                   MOVE 0059           TO WAHD-COD-MSG-ERRO
               ...
               IF  WRK-COD-MENSAGEM-0315   EQUAL 138
                   MOVE 1              TO WAHD-ERRO
                   MOVE ZEROS          TO WAHD-COD-SQL-ERRO
                   MOVE 0060           TO WAHD-COD-MSG-ERRO
                                          WRK-NRO-SEQ
               ...
               ELSE
                   IF  WRK-5507-COD-RETORNO
                       ...
                       MOVE WRK-5507-COD-SQL-ERRO
                                       TO WAHD-COD-SQL-ERRO
                       MOVE ZEROS      TO WAHD-COD-MSG-ERRO
                       MOVE WRK-5507-MSG-RETORNO
                                       TO WAHD-DESC-MSG-ERRO
                   END-IF
               END-IF
               MOVE SPACES             TO WAHD-DADOS
               INITIALIZE WAHD-DADOS
               PERFORM 3000-FINALIZAR
           END-IF.
      ...
      *----------------------------------------------------------------*
           ...
           IF  WAHC-TIPO-DATA        EQUAL ZEROS
               MOVE WAHC-FTR-DT-REFER  TO WRK-DATA-DD-MM-AAAA
               PERFORM 2310-FORMATAR-DT-WEB
               IF  WRK-ERRO-DATA       EQUAL 'S'
                   MOVE 1              TO WAHD-ERRO
                   MOVE ZEROS          TO WAHD-COD-SQL-ERRO
                   MOVE 0125           TO WAHD-COD-MSG-ERRO
                                          WRK-NRO-SEQ
                   MOVE WRK-NRO-SEQ-X  TO GFCTG2-COD-MSG
                   MOVE 'S'            TO WAHD-FIM
                   PERFORM 1110-OBTER-DESC-MSG
           ...
           IF (SQLCODE                 NOT EQUAL ZEROS) OR
              (SQLWARN0                EQUAL 'W')
               ...
               MOVE SQLCODE            TO GFCT0M-SQLCODE
                                          WRK-AUX-9-S
               MOVE WRK-AUX-9-3        TO WAHD-COD-SQL-ERRO
               MOVE '0010'             TO GFCT0M-LOCAL
               MOVE 0011               TO WAHD-COD-MSG-ERRO
                                          WRK-NRO-SEQ
               MOVE WRK-NRO-SEQ-X      TO GFCTG2-COD-MSG
      ...
      *----------------------------------------------------------------*
           ...
           MOVE SPACES                 TO WAHD-DESC-MSG-ERRO.

           STRING GFCTG3-DESC-MSG ' - '
                  WRK-NOME-TAB
           DELIMITED BY '   '          INTO WAHD-DESC-MSG-ERRO.
      ...
      *----------------------------------------------------------------*
           ...
           IF  WRK-FINALIZAR           NOT EQUAL 'S'
               MOVE CSERVC-TARIF OF GFCTB0D8
                                       TO WRK-AUX-5-S
               MOVE WRK-AUX-5          TO WAHD-PONTEIRO-TARIFA
           END-IF.
      ...
      *----------------------------------------------------------------*
           ...
           IF (SQLCODE                 NOT EQUAL ZEROS AND +100) OR
              (SQLWARN0                EQUAL 'W')
               ...
               MOVE 0006               TO WAHD-COD-MSG-ERRO
                                          WRK-NRO-SEQ
               MOVE WRK-NRO-SEQ-X      TO GFCTG2-COD-MSG
      ...
      *----------------------------------------------------------------*
           ...
           IF  IND-1                   EQUAL ZEROS
               MOVE ZEROS              TO WAHD-ERRO
                                          WAHD-COD-SQL-ERRO
               MOVE WRK-NRO-SEQ        TO WAHD-COD-MSG-ERRO
               MOVE WRK-NRO-SEQ-X      TO GFCTG2-COD-MSG
      ...
      *----------------------------------------------------------------*
           ...
           IF (SQLCODE                 NOT EQUAL ZEROS) OR
              (SQLWARN0                EQUAL 'W')
               ...
               MOVE 0005               TO WAHD-COD-MSG-ERRO
                                          WRK-NRO-SEQ
               MOVE WRK-NRO-SEQ-X      TO GFCTG2-COD-MSG
      ...
      *----------------------------------------------------------------*
           ...
           IF  SQLCODE                 EQUAL +100
               PERFORM 2421-TRATAR-FIM-CSR
           ELSE
               PERFORM 2521-OBTER-DESC-TARIFA
BI0115         IF (CTPO-SERVC-TARIF   EQUAL +2 OR +4)   AND
                  (CPSSOA-SERVC-TARIF EQUAL '3'  OR
                  (CPSSOA-SERVC-TARIF  NOT LESS
                                           WRK-CPSSOA-TARIF-INIC AND
                   CPSSOA-SERVC-TARIF  NOT GREATER
                                           WRK-CPSSOA-TARIF-FIM ))
                  MOVE CDEPDC OF GFCTB0D8 TO WRK-AUX-5-S
                  IF (WRK-5507-IDTFD-MASTER
                                       EQUAL 'N' AND
                      WRK-COD-JUNCAO-0315 NOT EQUAL WRK-AUX-5)
      ...
      *----------------------------------------------------------------*
       2521-OBTER-DESC-TARIFA         SECTION.
      *----------------------------------------------------------------*
           ...
           IF (SQLCODE                 NOT EQUAL ZEROS AND +100) OR
              (SQLWARN0                EQUAL 'W')
               ...
               MOVE 0009               TO WAHD-COD-MSG-ERRO
                                          WRK-NRO-SEQ
               MOVE WRK-NRO-SEQ-X      TO GFCTG2-COD-MSG
               MOVE 'GFCTB0D8'         TO WRK-NOME-TAB
               PERFORM 2321-MOVER-ERRO-SQL
           ...
           IF  SQLCODE                 EQUAL +100
               ...
               IF  IND-1               EQUAL ZEROS
                   MOVE 1              TO WAHD-ERRO
                   MOVE ZEROS          TO WAHD-COD-SQL-ERRO
                   MOVE 0242           TO WAHD-COD-MSG-ERRO
                                          WRK-NRO-SEQ
                   MOVE WRK-NRO-SEQ-X  TO GFCTG2-COD-MSG
                   PERFORM 1110-OBTER-DESC-MSG
                   MOVE GFCTG3-DESC-MSG  TO WAHD-DESC-MSG-ERRO
      ...
      *----------------------------------------------------------------*
           ...
           PERFORM 2531-ACESSAR-GFCTB0M0

           IF  WRK-MONTA            EQUAL 'S'

               ADD 107                 TO WAHD-LL

               MOVE CSERVC-TARIF       OF GFCTB0D8
                                       TO WRK-AUX-5-S
               MOVE WRK-AUX-5          TO WAHD-COD-TARIFA   (IND-1)
               MOVE RSERVC-TARIF-REDZD OF GFCTB0D8
                                       TO WAHD-DESC-TARIFA  (IND-1)
               MOVE WRK-TIPO-VIGENCIA  TO WAHD-TIPO-VIGENCIA(IND-1)
               MOVE CPSSOA-SERVC-TARIF TO WRK-PSSOA-X
               MOVE WRK-PSSOA          TO WAHD-TIPO-PSSOA   (IND-1)

               ADD 1                   TO WRK-QTDE-TOT-REG
                                          WRK-QTDE-OCOR
               MOVE  WRK-QTDE-TOT-REG  TO WAHD-QTDE-TOT-REG
               MOVE  WRK-QTDE-OCOR     TO WAHD-QTDE-OCOR
           ELSE
      ...
      *----------------------------------------------------------------*
       2531-ACESSAR-GFCTB0M0          SECTION.
      *----------------------------------------------------------------*

           MOVE CSERVC-TARIF OF GFCTB0A2
                                       TO CSERVC-TARIF OF GFCTB0M0
MAI10      MOVE DPROCM-ATUAL           TO DINIC-PRMSS-PCOTE OF GFCTB0M0
           MOVE DPROCM-ATUAL           TO DFIM-PRMSS-PCOTE OF GFCTB0M0
           ...
           EXEC SQL  SELECT
                ...
                INTO
                      ...
                      :GFCTB0M0.DINIC-PRMSS-PCOTE,
                      :GFCTB0M0.DFIM-PRMSS-PCOTE
                FROM   DB2PRD.TPRMSS_ADSAO_CORSP
                WHERE  CSERVC_TARIF       = :GFCTB0M0.CSERVC-TARIF
MAI10             AND  DINIC_PRMSS_PCOTE <= :GFCTB0M0.DINIC-PRMSS-PCOTE
...
BI0115         IF (CTPO-SERVC-TARIF   EQUAL +2 OR +4)   AND
      ...
      *----------------------------------------------------------------*
           ...
           IF  WRK-QTDE-TOT-REG     EQUAL  ZEROS
               MOVE 1                  TO WAHD-ERRO
               MOVE ZEROS              TO WAHD-COD-SQL-ERRO
               MOVE  1589              TO WAHD-COD-MSG-ERRO
               MOVE '1589'             TO GFCTG2-COD-MSG
               MOVE 'S'                TO WAHD-FIM
               PERFORM 1110-OBTER-DESC-MSG
               MOVE GFCTG3-DESC-MSG    TO WAHD-DESC-MSG-ERRO
               PERFORM 3000-FINALIZAR
