      ...
      *----------------------------------------------------------------*
       ...
       77  WRK-SEGMENTO-ANT            PIC  9(003)         VALUE ZEROS.
       ...
       01  WRK-GFCTRY-SEGTO-GFCT       PIC  9(005)         VALUE ZEROS.
       01  FILLER                      REDEFINES WRK-GFCTRY-SEGTO-GFCT.
           05  FILLER                  PIC  9(002).
           05  WRK-COD-SEGTO-GFCT      PIC  9(003).

       01  WRK-CSGMTO-GSTAO-TARIF      PIC S9(003) COMP-3  VALUE ZEROS.
       01  FILLER                      REDEFINES WRK-CSGMTO-GSTAO-TARIF.
           05  WRK-COD-SGMTO-GSTAO-TARIF
                                       PIC  9(003) COMP-3.

       01  WRK-CSGMTO-GSTAO-TARIF-NULL PIC S9(004) COMP    VALUE +0.
      ...
      *----------------------------------------------------------------*
       ...
       01  TAB-1-SEGMENTO.
           05  TAB-1-SEGMENTO-N        PIC  9(003)         VALUE ZEROS.
      ...
      *---------------------------------------------------------------*
           ...
           EXEC SQL
             DECLARE CSR01-GFCTB007-JN CURSOR FOR
             SELECT
                   A.CSERVC_TARIF,
                   A.CAGPTO_CTA,
                   A.CSEQ_AGPTO_CTA,
...
PWI                                         WRK-INICIO.
      ...
      *----------------------------------------------------------------*
           ...
           ELSE
               ...
               IF  WRK-CSGMTO-GSTAO-TARIF-NULL
                                       LESS ZEROS
                   MOVE ZEROS          TO CSGMTO-GSTAO-TARIF OF GFCTB007
               END-IF
           END-IF.
      ...
      *----------------------------------------------------------------*
           ...
           MOVE CSGMTO-GSTAO-TARIF OF GFCTB007
                                       TO WRK-CSGMTO-GSTAO-TARIF.
           MOVE GFCTRY-SEGTO-GFCT      TO WRK-GFCTRY-SEGTO-GFCT.

           EVALUATE TRUE
             WHEN WRK-COD-SGMTO-GSTAO-TARIF
                                       LESS WRK-COD-SEGTO-GFCT
               PERFORM 3300-LER-CSR01-GFCTB007                     UNTIL
               WRK-COD-SGMTO-GSTAO-TARIF
                                       NOT LESS WRK-COD-SEGTO-GFCT

             WHEN WRK-COD-SGMTO-GSTAO-TARIF
                                       GREATER WRK-COD-SEGTO-GFCT
               PERFORM 2200-LER-CLIESEG                            UNTIL
               WRK-COD-SEGTO-GFCT      NOT LESS
                                               WRK-COD-SGMTO-GSTAO-TARIF

             WHEN WRK-COD-SGMTO-GSTAO-TARIF
                                       EQUAL WRK-COD-SEGTO-GFCT  AND
                  WRK-COD-SEGTO-GFCT   NOT EQUAL  999
               INITIALIZE TAB-1-SEGMENTO
                          IND-1
               MOVE WRK-COD-SGMTO-GSTAO-TARIF
                                       TO TAB-1-SEGMENTO-N
               PERFORM 3410-MONTA-TABELA-1                         UNTIL
                   WRK-COD-SGMTO-GSTAO-TARIF
                                       GREATER TAB-1-SEGMENTO-N
               PERFORM 3420-DESCARREGA-SEGMENTO                    UNTIL
                   WRK-COD-SEGTO-GFCT  GREATER TAB-1-SEGMENTO-N
           END-EVALUATE.
...
PWI   *----------------------------------------------------------------*
PWI    3422-1-OBTER-PERIODO              SECTION.
PWI   *----------------------------------------------------------------*
PWI
PWI        MOVE DPROCM-ANTER OF GFCTB0A1 TO WRK-DATA-AUX
PWI        MOVE WRK-DIA-AUX              TO WRK-DIA-AUX1
...
PWI
PWI        MOVE DINIC-ADSAO-COMP       OF GFCTB007
PWI                                      TO WRK-DATA-AUX
PWI        MOVE WRK-DIA-AUX              TO WRK-DIA-AUX1
PWI        MOVE WRK-MES-AUX              TO WRK-MES-AUX1
PWI        MOVE WRK-ANO-AUX              TO WRK-ANO-AUX1.
...
PWI           PERFORM 3422-2-MONTA-NO-MES
PWI        ELSE
PWI           PERFORM 3422-3-MONTA-FUTURO
PWI        END-IF.
PWI
PWI   *----------------------------------------------------------------*
PWI    3422-1-99-FIM.                 EXIT.
PWI   *----------------------------------------------------------------*
PWI
PWI   *----------------------------------------------------------------*
PWI    3422-2-MONTA-NO-MES            SECTION.
PWI   *----------------------------------------------------------------
PWI
PWI        MOVE DPROCM-ANTER OF GFCTB0A1 TO WRK-DATA-AUX.
...
PWI        ELSE
PWI           PERFORM 3422-4-OBTER-DFIM
PWI        END-IF.
PWI
PWI   *----------------------------------------------------------------*
PWI    3422-2-99-FIM.                 EXIT.
PWI   *----------------------------------------------------------------*
PWI   *----------------------------------------------------------------*
PWI    3422-3-MONTA-FUTURO            SECTION.
PWI   *----------------------------------------------------------------*
PWI
...
PWI
PWI   *----------------------------------------------------------------*
PWI    3422-3-99-FIM.                 EXIT.
PWI   *----------------------------------------------------------------*
PWI
PWI   *----------------------------------------------------------------*
PWI    3422-4-OBTER-DFIM              SECTION.
PWI   *----------------------------------------------------------------*
PWI
PWI        MOVE DPROCM-ANTER OF GFCTB0A1 TO WRK-DATA-AUX
PWI        MOVE WRK-DIA-AUX              TO WRK-DIA-AUX1
...
PWI   *----------------------------------------------------------------*
      ...
      ******************************************************************
      *    EFETUA EXIBICAO DE INFORMACOES DO PROCESSAMENTO.            *
      ******************************************************************
      *----------------------------------------------------------------*
      ...
      ******************************************************************
      *    ROTINA DE EXIBICAO DE INFORMACOES DE FINAIS DO PROCESSAMENTO*
      ******************************************************************
      *----------------------------------------------------------------*
