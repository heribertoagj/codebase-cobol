      ...
      *    DATA........:   07/06/2005                                  *
      *----------------------------------------------------------------*
      *    OBJETIVO....:   GERAR DADOS PARA DEMOSTRATIVO DE TARIFAS POR*
      *       AGENCIA/CONTA.                                           *
      *----------------------------------------------------------------*
      *-                      A L T E R A C A O                       -*
      *----------------------------------------------------------------*
      *    ANALISTA....:   WAGNER SILVA            - PROCWORK /  50    *
      *    DATA........:   25/11/2005                                  *
      *    OBJETIVO....:   ACERTO NO RESTART                           *
      *----------------------------------------------------------------*
      *-                      A L T E R A C A O                       -*
      *----------------------------------------------------------------*
      *    ANALISTA....:   WAGNER SILVA            - PROCWORK /  50    *
      *    DATA........:   18/01/2006                                  *
      *    OBJETIVO....:   RESTRINGIR ACESSO A PERIODOS DE UM UNICO MES*
      *----------------------------------------------------------------*
      *    BCO DE DADOS:                                               *
      *                DB2                                             *
      *                  TABLE                         INCLUDE/BOOK    *
      ...
      *                       =================                        *
      *                                                                *
      * RESPONSAVEL.:FLAVIO ALVES GERADE - CPM BRAXIS                  *
      * DATA........:28/06/2011                                        *
      * OBJETIVO....:MELHORIA DE PERFORMANCE - INCLUSAO DA COLUNA      *
      *              CBCO_DSTNO_MOVTO NO CURSOR CRS-GFCTB0G7.          *
      *                                                                *
      *================================================================*
...
HEXA  *
      ...
      *----------------------------------------------------------------*
       77  FILLER                      PIC  X(050)         VALUE
           '*** GFCT0489 - INICIO DA AREA DE WORKING ***'.
      *----------------------------------------------------------------*
           EJECT
      *----------------------------------------------------------------*
       77  FILLER                      PIC  X(050)         VALUE
...
ST25X6     05  WRK-CNPJ-CON-0315       PIC 9(02)           VALUE ZEROS.
      ...
      *----------------------------------------------------------------*
           ...
           EXEC SQL
             DECLARE   CRS-GFCTB0G7   CURSOR   FOR
               SELECT
                   T1.CAG_DSTNO_MOVTO,
                   T1.CCTA_DSTNO_MOVTO,
                   T1.CSEQ_MOVTO,
                   ...
                   T1.VTARIF_ESTRN_CLI,
                   T1.VTARIF_PERDA_CLI
             FROM  DB2PRD.TMOVTO_TARIF_CTA T1
             WHERE
                 ((T1.CBCO_DSTNO_MOVTO   = 237)
             AND  (T1.CAG_DSTNO_MOVTO    = :GFCTB0G7.CAG-DSTNO-MOVTO
             AND   T1.CCTA_DSTNO_MOVTO   = :GFCTB0G7.CCTA-DSTNO-MOVTO)
...
HEXA  *
      ...
      *----------------------------------------------------------------*
       01  FILLER                      PIC  X(050)         VALUE
           '*** GFCT0489 - FIM DA AREA DE WORKING ***'.
      *----------------------------------------------------------------*
           EJECT
      *----------------------------------------------------------------*
       LINKAGE                         SECTION.
...
ST25X6*        GFCT9W-AGENCIA              NOT NUMERIC      OR
ST25X6         GFCT9W-AGENCIA              EQUAL SPACES     OR 
ST25X6                                     EQUAL LOW-VALUES OR
               GFCT9W-AGENCIA              EQUAL ZEROS      OR
ST25X6*        GFCT9W-CONTA                NOT NUMERIC      OR
ST25X6         GFCT9W-CONTA                EQUAL SPACES     OR 
ST25X6                                     EQUAL LOW-VALUES OR
      ...
      *------> SO PERMITE ACESSO A DADOS DE UM UNICO MES - 18.01.2006
           IF  WRK-ANOMES-INI           NOT = WRK-ANOMES-FIM
               MOVE 1                      TO GFCT9Y-ERRO
               MOVE 0577                   TO GFCT9Y-COD-MSG-ERRO
                                              WRK-5000-COD-MSG
               MOVE 'S'                    TO GFCT9Y-FIM
               MOVE ZEROS                  TO GFCT9Y-COD-SQL-ERRO
               PERFORM 111000-OBTER-DESCRICAO-MSG
      ...
      *----------------------------------------------------------------*
           ...
           PERFORM 210000-ABRIR-CRS-GFCTB0G7.

           PERFORM 220000-LER-CRS-GFCTB0G7
           ...
           PERFORM 240000-FECHAR-CRS-GFCTB0G7.
...
HEXA  *
      ...
      *--------------->> VERIFICA SE USUARIO E GER OU DIRET REGIONAL
           ...
           IF (SQLCODE                 NOT EQUAL ZEROS AND +100
                                                       AND -811) OR
               SQLWARN0                EQUAL 'W'
               MOVE 'GFCT0489'         TO GFCT0M-PROGRAMA
      ...
      *----------------------------------------------------------------*
       210000-ABRIR-CRS-GFCTB0G7       SECTION.
      *----------------------------------------------------------------*

           MOVE GFCT9W-AGENCIA         TO CAG-DSTNO-MOVTO  OF GFCTB0G7.
           MOVE GFCT9W-CONTA           TO CCTA-DSTNO-MOVTO OF GFCTB0G7.
      ...
      *----------------------------------------------------------------*
           EJECT
      *----------------------------------------------------------------*
       220000-LER-CRS-GFCTB0G7         SECTION.
      *----------------------------------------------------------------*

           EXEC SQL
             FETCH   CRS-GFCTB0G7   INTO
                   :GFCTB0G7.CAG-DSTNO-MOVTO,
                   :GFCTB0G7.CCTA-DSTNO-MOVTO,
                   :GFCTB0G7.CSEQ-MOVTO,
      ...
      *----------------------------------------------------------------*
       240000-FECHAR-CRS-GFCTB0G7      SECTION.
      *----------------------------------------------------------------*
