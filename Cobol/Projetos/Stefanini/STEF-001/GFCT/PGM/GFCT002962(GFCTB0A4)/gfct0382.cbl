      ...
      *                                                                *
      *    OBJETIVO     :                                              *
      *      LISTA DE ESTORNOS DE AGRUPAMENTOS - PARA DEFERIMENTO.     *
      *                                                                *
      *    BANCO DE DADOS:                                             *
      *      DB2                                                       *
      *       TABLE                                INCLUDE/BOOK        *
      ...
      *      GFCT5507 - VALORIZAR O CAMPO IDTFD-MASTER                 *
...
0410  *                                                                *
      ...
      *----------------------------------------------------------------*
       ...
       01  WRK-ACHOU-PROX-VALIDO       PIC X(01)           VALUE SPACES.
       ...
       01  WRK-IO-PCB.
           03 WRK-IO-LTERM             PIC X(08).
           03 FILLER                   PIC X(02).
           03 WRK-IO-STATUS            PIC X(02).
           03 FILLER                   PIC X(12).
           03 WRK-IO-MODNAME           PIC X(08).

       01  WRK-ALT-PCB.
           03 WRK-ALT-LTERM            PIC X(08).
           03 FILLER                   PIC X(02).
           03 WRK-ALT-STATUS           PIC X(02).
           03 FILLER                   PIC X(12).
           03 WRK-ALT-MODNAME          PIC X(08).
      ...
      *----------------------------------------------------------------*
       ...
       01  WRK-5505-AREA-ENTRADA.
           03  WRK-5505-AMBIENTE       PIC X(01).
           03  WRK-5505-FUNC-BDSCO     PIC X(07).

       01  WRK-5505-AREA-SAIDA.
           03  WRK-5505-COD-RETORNO    PIC 9(02).
           03  WRK-5505-COD-SQL-ERRO   PIC 9(03).
           03  WRK-5505-DESC-MSG       PIC X(70).
           03  WRK-5505-DADOS-RETORNO.
               05  WRK-5505-COD-JUNCAO PIC 9(05).
...
0410   COPY 'I#GFCTF9'.
      ...
      *----------------------------------------------------------------*
           ...
           EXEC SQL
               ...
               WHERE   CSIT_SOLTC_ESTRN = :GFCTB0A7.CSIT-SOLTC-ESTRN
                 AND   CSIT_PROCM_ESTRN = :GFCTB0A7.CSIT-PROCM-ESTRN
      ...
      *================================================================*
       PROCEDURE DIVISION              USING GFCTGH-ENTRADA
                                             GFCTGI-SAIDA
                                             GFCT0M-AREA-ERROS.
      *================================================================*
      ...
      *----------------------------------------------------------------*
           ...
           INITIALIZE                  GFCTGI-SAIDA
                                       GFCT0M-AREA-ERROS
                                       SQLCA.

           MOVE 290                    TO GFCTGI-LL.
           MOVE GFCTGH-TRANSACAO       TO GFCTGI-TRANSACAO.
           MOVE GFCTGH-FUNCAO          TO GFCTGI-FUNCAO.
           MOVE GFCTGH-FUNC-BDSCO      TO GFCTGI-FUNC-BDSCO.
           MOVE GFCTGH-FILTRO-AGPTO    TO GFCTGI-FILTRO-AGPTO.
           MOVE GFCTGH-FILTRO-TARIFA   TO GFCTGI-FILTRO-TARIFA.
           MOVE GFCTGH-PONT-AGPTO      TO GFCTGI-PONT-AGPTO.
           MOVE GFCTGH-PONT-TARIFA     TO GFCTGI-PONT-TARIFA.
           MOVE GFCTGH-PONT-DATA       TO GFCTGI-PONT-DATA.
           MOVE GFCTGH-PONT-TIMESTAMP  TO GFCTGI-PONT-TIMESTAMP.
           MOVE ZEROS                  TO GFCTGI-ERRO
                                          GFCTGI-COD-SQL-ERRO
                                          GFCTGI-COD-MSG-ERRO
                                          WRK-5000-COD-MSG.
           MOVE 'N'                    TO GFCTGI-FIM
                                          WRK-FINALIZAR.

           IF  GFCTGH-QTDE-OCOR        IS NUMERIC
               MOVE GFCTGH-QTDE-OCOR   TO GFCTGI-QTDE-OCOR
           ELSE
               MOVE ZEROS              TO GFCTGI-QTDE-OCOR
           END-IF.

           IF  GFCTGH-QTDE-TOT-REG     IS NUMERIC
               MOVE GFCTGH-QTDE-TOT-REG
                                       TO GFCTGI-QTDE-TOT-REG
           ELSE
               MOVE ZEROS              TO GFCTGI-QTDE-TOT-REG
           END-IF.

           PERFORM 1100000-OBTER-DESC-MSG.

           MOVE WRK-5000-DESC-MSG      TO GFCTGI-DESC-MSG-ERRO.
      ...
      *----------------------------------------------------------------*
           ...
           IF  WRK-5000-COD-RETORNO    NOT EQUAL ZEROS
               IF  WRK-5000-COD-RETORNO
                                       EQUAL 01
                   MOVE ZEROS          TO GFCTGI-COD-SQL-ERRO
                                          GFCTGI-COD-MSG-ERRO
               END-IF
               IF  WRK-5000-COD-RETORNO
                                       EQUAL 99
                   MOVE WRK-5000-COD-SQL-ERRO
                                       TO GFCTGI-COD-SQL-ERRO
                   MOVE ZEROS          TO GFCTGI-COD-MSG-ERRO
               END-IF
               MOVE 1                  TO GFCTGI-ERRO
               MOVE 'S'                TO GFCTGI-FIM
               MOVE WRK-5000-DESC-MSG  TO GFCTGI-DESC-MSG-ERRO
               PERFORM 3000000-FINALIZAR
           END-IF.
      ...
      *----------------------------------------------------------------*
           ...
           IF (GFCTGH-TRANSACAO        EQUAL SPACES OR LOW-VALUES) OR
              (GFCTGH-FUNCAO           EQUAL SPACES OR LOW-VALUES) OR
              (GFCTGH-FUNC-BDSCO       EQUAL SPACES OR LOW-VALUES) OR
              (GFCTGH-FILTRO-AGPTO     NOT NUMERIC)                OR
              (GFCTGH-FILTRO-AGPTO     EQUAL ZEROS)                OR
              (GFCTGH-FILTRO-AGPTO     NOT EQUAL 4 AND 14 AND 9
                                       AND 15 AND 16 AND 17)       OR
              (GFCTGH-FILTRO-TARIFA    NOT NUMERIC)                OR
              (GFCTGH-PONT-AGPTO       NOT NUMERIC)                OR
              (GFCTGH-PONT-AGPTO       EQUAL ZEROS)                OR
              (GFCTGH-PONT-AGPTO       NOT EQUAL 4 AND 14 AND 9
                                       AND 15 AND 16 AND 17)       OR
              (GFCTGH-PONT-TARIFA      NOT NUMERIC)
               MOVE 1                  TO GFCTGI-ERRO
               MOVE ZEROS              TO GFCTGI-COD-SQL-ERRO
               MOVE 0001               TO GFCTGI-COD-MSG-ERRO
                                          WRK-5000-COD-MSG
               MOVE 'S'                TO GFCTGI-FIM
               PERFORM 1100000-OBTER-DESC-MSG
               MOVE WRK-5000-DESC-MSG  TO GFCTGI-DESC-MSG-ERRO
      ...
      *----------------------------------------------------------------*
           ...
           IF  WRK-5500-COD-RETORNO    NOT EQUAL ZEROS
               IF  WRK-5500-COD-RETORNO
                   ...
                   MOVE 'S'            TO GFCTGI-FIM
                   MOVE ZEROS          TO GFCTGI-COD-SQL-ERRO
                   MOVE 0030           TO GFCTGI-COD-MSG-ERRO
                                          WRK-5000-COD-MSG
                   PERFORM 1100000-OBTER-DESC-MSG
                   MOVE WRK-5000-DESC-MSG
                                       TO GFCTGI-DESC-MSG-ERRO
               END-IF
               IF  WRK-5500-COD-RETORNO
                                       EQUAL 99
                   ...
                   MOVE 0012           TO GFCTGI-COD-MSG-ERRO
                                          WRK-5000-COD-MSG
                   MOVE 'GFCT0382'     TO GFCT0M-PROGRAMA
                   MOVE 'DB2'          TO GFCT0M-TIPO-ACESSO
                   MOVE SQLCA          TO GFCT0M-SQLCA-AREA
                   MOVE 9              TO GFCTGI-ERRO
      ...
      *----------------------------------------------------------------*
           ...
           MOVE SPACES                 TO GFCTGI-DESC-MSG-ERRO.

           STRING WRK-5000-DESC-MSG ' - '
                  WRK-NOME-TAB
           DELIMITED BY '   '          INTO GFCTGI-DESC-MSG-ERRO.
      ...
      *----------------------------------------------------------------*
           ...
           PERFORM 2301000-OBTER-COD-JUNCAO.

           PERFORM 2302000-VERIFICAR-MASTER.

           IF  GFCTGH-FILTRO-TARIFA    EQUAL ZEROS
               ...
               PERFORM 2350000-FECHAR-CSR1-GFCTB0A7
           ELSE
               PERFORM 2360000-ABRIR-CSR2-GFCTB0A7

               PERFORM 2370000-LER-CSR2-GFCTB0A7
               ...
               PERFORM 2380000-FECHAR-CSR2-GFCTB0A7
           END-IF.
      ...
      *----------------------------------------------------------------*
       2301000-OBTER-COD-JUNCAO        SECTION.
      *----------------------------------------------------------------*

           INITIALIZE                  WRK-5505-AREA-SAIDA.

           MOVE 'O'                    TO WRK-5505-AMBIENTE.
           MOVE GFCTGH-FUNC-BDSCO      TO WRK-5505-FUNC-BDSCO.
           ...
           IF  WRK-5505-COD-RETORNO    NOT EQUAL ZEROS
               MOVE 1                  TO GFCTGI-ERRO
               MOVE ZEROS              TO GFCTGI-COD-SQL-ERRO
               MOVE WRK-5505-DESC-MSG  TO GFCTGI-DESC-MSG-ERRO
               MOVE 'S'                TO GFCTGI-FIM
               PERFORM 3000000-FINALIZAR
           END-IF.
      ...
      *----------------------------------------------------------------*
       2302000-VERIFICAR-MASTER        SECTION.
      *----------------------------------------------------------------*

0410       INITIALIZE                  GFCTF9-GFCT5515.
0410       MOVE GFCTGH-FUNC-BDSCO      TO GFCTF9-COD-FUNC.
...
0410           MOVE 1                  TO GFCTGI-ERRO
0410           MOVE ZEROS              TO GFCTGI-COD-SQL-ERRO
0410           MOVE GFCTF9-DESC-MSG    TO GFCTGI-DESC-MSG-ERRO
0410           MOVE 'S'                TO GFCTGI-FIM
0410           PERFORM 3000000-FINALIZAR
0410       END-IF.
      ...
      *----------------------------------------------------------------*
           ...
           IF  GFCTGH-PONT-AGPTO       NOT EQUAL ZEROS
               MOVE GFCTGH-PONT-AGPTO  TO CAGPTO-CTA       OF GFCTB0A7
           ELSE
               MOVE GFCTGH-FILTRO-AGPTO
                                       TO CAGPTO-CTA       OF GFCTB0A7
           END-IF.

           IF  GFCTGH-PONT-TARIFA      NOT EQUAL ZEROS
               MOVE GFCTGH-PONT-TARIFA TO CSERVC-TARIF     OF GFCTB0A7
           ELSE
               MOVE GFCTGH-FILTRO-TARIFA
                                       TO CSERVC-TARIF     OF GFCTB0A7
           END-IF.

           IF  GFCTGH-PONT-DATA        EQUAL SPACES OR LOW-VALUES
               MOVE '01.01.0001'       TO DOCOR-EVNTO      OF GFCTB0A7
           ELSE
               MOVE GFCTGH-PONT-DATA   TO DOCOR-EVNTO      OF GFCTB0A7
           END-IF.

           IF  GFCTGH-PONT-TIMESTAMP   EQUAL SPACES OR LOW-VALUES
               MOVE '0001-01-01-01.01.01.000001'
                                       TO HIDTFD-ESTRN-AGPTO
                                                            OF GFCTB0A7
           ELSE
               MOVE GFCTGH-PONT-TIMESTAMP
                                       TO HIDTFD-ESTRN-AGPTO
                                                            OF GFCTB0A7
           END-IF.
           ...
           IF (SQLCODE                 NOT EQUAL ZEROS) OR
              (SQLWARN0                EQUAL 'W')
               ...
               MOVE 0005               TO GFCTGI-COD-MSG-ERRO
                                          WRK-5000-COD-MSG
      ...
      *----------------------------------------------------------------*
           ...
           IF (SQLCODE                 NOT EQUAL ZEROS AND +100) OR
              (SQLWARN0                EQUAL 'W')
               ...
               MOVE 0006               TO GFCTGI-COD-MSG-ERRO
                                          WRK-5000-COD-MSG
               MOVE 'GFCTB0A7'         TO WRK-NOME-TAB
               PERFORM 2210000-MOVER-ERRO-SQL
               PERFORM 3000000-FINALIZAR
           ...
           IF  SQLCODE                 EQUAL +100
               ...
               IF  IND                 EQUAL ZEROS
                   MOVE 1              TO GFCTGI-ERRO
                   MOVE ZEROS          TO GFCTGI-COD-SQL-ERRO
                   MOVE 0484           TO GFCTGI-COD-MSG-ERRO
                                          WRK-5000-COD-MSG
                   PERFORM 1100000-OBTER-DESC-MSG
                   MOVE WRK-5000-DESC-MSG
                                       TO GFCTGI-DESC-MSG-ERRO
...
0410                                EQUAL 'GM' OR 'DA'
               ...
               PERFORM 2331000-MOVER-CAMPOS
           ELSE
               IF  CDEPDC OF GFCTB0D8  EQUAL WRK-5505-COD-JUNCAO
                   ADD 1               TO IND
           ...
           IF  IND                     EQUAL 4
0410           IF  GFCTF9-TIPO-DEPENDENCIA
0410                                   NOT EQUAL 'GM' OR 'DA'
                   MOVE  'N'               TO  WRK-ACHOU-PROX-VALIDO

                   PERFORM  2340000-BUSCAR-PROX-VALIDO
                     UNTIL  WRK-FINALIZAR
                                       EQUAL 'S'
                        OR  WRK-ACHOU-PROX-VALIDO
                                       EQUAL 'S'
               END-IF
               ...
               IF  WRK-FINALIZAR       NOT EQUAL 'S'
                   ...
                   MOVE CAGPTO-CTA   OF GFCTB0A7
                                       TO GFCTGI-PONT-AGPTO
                   MOVE CSERVC-TARIF OF GFCTB0A7
                                       TO GFCTGI-PONT-TARIFA
                   MOVE DOCOR-EVNTO  OF GFCTB0A7
                                       TO GFCTGI-PONT-DATA
                   MOVE HIDTFD-ESTRN-AGPTO OF GFCTB0A7
                                       TO GFCTGI-PONT-TIMESTAMP
               END-IF
           END-IF.
      ...
      *----------------------------------------------------------------*
       2331000-MOVER-CAMPOS            SECTION.
      *----------------------------------------------------------------*

           ADD 180                     TO GFCTGI-LL.

           EVALUATE GFCTGH-PONT-AGPTO
              WHEN 15
           ...
           MOVE CSERVC-TARIF       OF GFCTB0A7
                                       TO GFCTGI-TARIFA(IND).
           MOVE RSERVC-TARIF-REDZD OF GFCTB0D8
                                       TO GFCTGI-DESC-TARIFA(IND).
           MOVE DOCOR-EVNTO        OF GFCTB0A7
                                       TO GFCTGI-DATA(IND).
           MOVE HIDTFD-ESTRN-AGPTO OF GFCTB0A7
                                       TO GFCTGI-HORA(IND).
      ...
      *----------------------------------------------------------------*
           ...
           MOVE GFCTGH-TRANSACAO       TO GFCTGB-TRANSACAO.
           MOVE GFCTGH-FUNCAO          TO GFCTGB-FUNCAO.
           MOVE ZEROS                  TO GFCTGB-QTDE-OCOR.
           MOVE ZEROS                  TO GFCTGB-QTDE-TOT-REG.
           MOVE GFCTGH-FUNC-BDSCO      TO GFCTGB-FUNC-BDSCO.
           ...
           IF  RETURN-CODE             NOT EQUAL ZEROS
               ...
               MOVE 'GFCT0382'         TO GFCT0M-TRANSACAO
           ...
           IF  GFCTGC-ERRO             NOT EQUAL ZEROS
               MOVE 'S'                TO GFCTGI-FIM
               MOVE GFCTGC-ERRO        TO GFCTGI-ERRO
               MOVE GFCTGC-COD-SQL-ERRO
                                       TO GFCTGI-COD-SQL-ERRO
               MOVE GFCTGC-COD-MSG-ERRO
                                       TO GFCTGI-COD-MSG-ERRO
               ...
               ELSE
                   ...
                   DELIMITED BY SIZE   INTO GFCTGI-DESC-MSG-ERRO
               END-IF
      ...
      *----------------------------------------------------------------*
           ...
           IF  CINDCD-AGPTO-TOT OF GFCTB0A7
               ...
               MOVE 'TODOS'            TO GFCTGI-DESC-NOME(IND)
           ELSE
               MOVE CDEPDC OF GFCTB0A3 TO GFCTGI-AGENCIA(IND)
      ...
      *----------------------------------------------------------------*
           ...
           MOVE GFCTGH-TRANSACAO       TO GFCT1V-TRANSACAO.
           MOVE GFCTGH-FUNCAO          TO GFCT1V-FUNCAO.
           MOVE 001                    TO GFCT1V-QTDE-OCOR.
           MOVE ZEROS                  TO GFCT1V-QTDE-TOT-REG.
           MOVE GFCTGH-FUNC-BDSCO      TO GFCT1V-FUNC-BDSCO.
           ...
           IF  GFCT1X-ERRO             NOT EQUAL ZEROS
               MOVE 'S'                TO GFCTGI-FIM
               MOVE GFCT1X-ERRO        TO GFCTGI-ERRO
...
ST25X6*                                TO GFCTGI-CPF-CNPJ(IND).
ST25X6*    MOVE CFLIAL-CGC       OF GFCTB0A4
ST25X6*                                TO GFCTGI-FILIAL(IND).
ST25X6*    MOVE CCTRL-CPF-CGC    OF GFCTB0A4
ST25X6*                                TO GFCTGI-CONTROLE(IND).
...
ST25X6                                 TO GFCTGI-CONTROLE(IND).
      ...
      *----------------------------------------------------------------*
       2340000-BUSCAR-PROX-VALIDO      SECTION.
      *----------------------------------------------------------------*
...
0410       IF  GFCTF9-TIPO-DEPENDENCIA
0410                                   EQUAL 'GM' OR 'DA'
               MOVE 'S'                TO WRK-ACHOU-PROX-VALIDO
           ELSE
               IF  CDEPDC OF GFCTB0D8  EQUAL WRK-5505-COD-JUNCAO
                   MOVE 'S'            TO WRK-ACHOU-PROX-VALIDO
               ELSE
                   IF  GFCTGH-FILTRO-TARIFA
                                       EQUAL ZEROS
                       PERFORM 2320000-LER-CSR1-GFCTB0A7
                   ELSE
                       PERFORM 2370000-LER-CSR2-GFCTB0A7
      ...
      *----------------------------------------------------------------*
       2350000-FECHAR-CSR1-GFCTB0A7    SECTION.
      *----------------------------------------------------------------*
           ...
           IF (SQLCODE                 NOT EQUAL ZEROS) OR
              (SQLWARN0                EQUAL 'W')
               ...
               MOVE 0011               TO GFCTGI-COD-MSG-ERRO
                                          WRK-5000-COD-MSG
               MOVE 'GFCTB0A7'         TO WRK-NOME-TAB
               PERFORM 2210000-MOVER-ERRO-SQL
               PERFORM 3000000-FINALIZAR
      ...
      *----------------------------------------------------------------*
       2360000-ABRIR-CSR2-GFCTB0A7     SECTION.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       2370000-LER-CSR2-GFCTB0A7       SECTION.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       2380000-FECHAR-CSR2-GFCTB0A7    SECTION.
      *----------------------------------------------------------------*
