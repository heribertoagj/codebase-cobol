...
______*____MOVE_BC0-CONTA_OF_ENTRADA________TO_WRK-CONTA-ATU.
      ...
      *----------------------------------------------------------------*
           ...
           IF (BC0-AGENCIA OF ENTRADA NUMERIC AND
              ...
              (BC0-CENTRO-CUSTO OF ENTRADA NOT = SPACES)
              NEXT SENTENCE
           ELSE
              MOVE 'S'                   TO WRK-ERRO
           ...
           IF WRK-ERRO = 'N'
              IF WRK-CESTA-CELULAR = 'S'
                 MOVE 'S'                   TO WRK-ERRO
                 MOVE 'CESTA CELULAR NAO PERMITIDA'
                 TO WRK-MSG
              ELSE
                 IF WRK-CESTA-CELULAR = 'E'
                    MOVE 'S'                   TO WRK-ERRO
                    MOVE 'CESTA NAO EXISTE OU FORA DE VIGENCIA'
                    TO WRK-MSG
                 END-IF
              END-IF
           END-IF.
           ...
           IF WRK-ERRO = 'N'
              IF (BC0-OPERADORA   OF ENTRADA EQUAL SPACES AND
                  BC0-DDD         OF ENTRADA EQUAL SPACES AND
                  BC0-NUM-CEL     OF ENTRADA EQUAL SPACES)      OR
                 (BC0-OPERADORA   OF ENTRADA NUMERIC      AND
                  BC0-DDD         OF ENTRADA NUMERIC      AND
                  BC0-NUM-CEL     OF ENTRADA NUMERIC      AND
                  BC0-OPERADORA-R OF ENTRADA EQUAL ZEROS  AND
                  BC0-DDD-R       OF ENTRADA EQUAL ZEROS  AND
                  BC0-NUM-CEL-R   OF ENTRADA EQUAL ZEROS)
                  NEXT SENTENCE
              ELSE
                 MOVE 'S'                   TO WRK-ERRO
                 MOVE 'DADOS DO NUM. CELULAR INVALIDOS'
                 TO WRK-MSG
              END-IF
           END-IF.
      ...
      *----------------------------------------------------------------*
           ...
           IF SQLCODE EQUAL +100
              MOVE 'S'                   TO WRK-ERRO
              MOVE 'CESTA NAO ENCONTRADA NO CADASTRO'
              TO WRK-MSG
           ELSE
...
______*___________END-IF
      ...
      *----------------------------------------------------------------*
           ...
           MOVE 'GFCT3602'                 TO WRK-5527-CHAMADOR.
...
______*----------------------------------------------------------------*
      ...
      ******************************************************************
       3455-ATUALIZA-CONTROLE SECTION.
           ...
           MOVE 1                 TO BC6-CTPO-OPER-MASSV
                                     BC6-CSIT-PROCM-MASSV.
...
______*____ROTINA_DE_EMISSAO_DE_DISPLAY_DE_TOTAIS______________________*
      ...
      ******************************************************************
       4210-PROCESSA-CURSOR-H3 SECTION.
           ...
           IF SQLCODE = +100
______*_______PERFORM_4230-CLOSE-CURSOR-H3
______*_______MOVE_'S'_TO_WRK-FIM-CURSOR-H3
              GO TO 4210-99-FIM
           END-IF.
